...
BRUSHED.listenerMenu=function(){$('#mobile-nav').on('click',function(e){$(this).toggleClass('open');if($('#mobile-nav').hasClass('open')){$('#navigation-mobile').slideDown(500,'easeOutExpo')}else{$('#navigation-mobile').slideUp(500,'easeOutExpo')}
...
BRUSHED.slider=function(){$.supersized({slideshow:1,autoplay:1,start_slide:1,stop_loop:0,random:0,slide_interval:12000,transition:1,transition_speed:300,new_window:1,pause_hover:0,keyboard_nav:1,performance:1,image_protect:1,min_width:0,min_height:0,vertical_center:1,horizontal_center:1,fit_always:0,fit_portrait:1,fit_landscape:0,slide_links:'blank',thumb_links:0,thumbnail_navigation:0,slides:[{image:'_include/img/slider-images/slide/PUBG.jpg',title:'',thumb:'',url:''},{image:'_include/img/slider-images/slide/dirt.jpg',title:'',thumb:'',url:''},{image:'_include/img/slider-images/slide/far_cry_primal.jpg',title:'<div style="font-size:72px">F A R   <span style="color:#c14040;">C R Y</span></div>',thumb:'',url:''},{image:'_include/img/slider-images/slide/dota.jpg',title:'',thumb:'',url:''}],progress_bar:0,mouse_scrub:0})}
...
BRUSHED.contactForm=function(){$("#contact-submit").on('click',function(){$contact_form=$('#contact-form');var fields=$contact_form.serialize();$.ajax({type:"POST",url:"_include/php/contact.php",data:fields,dataType:'json',success:function(response){if(response.status){$('#contact-form input').val('');$('#contact-form textarea').val('')}
$('#response').empty().html(response.html)}});return!1})}
BRUSHED.tweetFeed=function(){var valueTop=-64;$("#ticker").tweet({modpath:'_include/js/twitter/',username:"Bluxart",page:1,avatar_size:0,count:10,template:"{text}{time}",filter:function(t){return!/^@\w+/.test(t.tweet_raw_text)},loading_text:"loading ..."}).bind("loaded",function(){var ul=$(this).find(".tweet_list");var ticker=function(){setTimeout(function(){ul.find('li:first').animate({marginTop:valueTop+'px'},500,'linear',function(){$(this).detach().appendTo(ul).removeAttr('style')});ticker()},5000)};ticker()})}
BRUSHED.menu=function(){$('#menu-nav, #menu-nav-mobile').onePageNav({currentClass:'current',changeHash:!1,scrollSpeed:750,scrollOffset:30,scrollThreshold:0.5,easing:'easeOutExpo',filter:':not(.external)'})}
BRUSHED.goSection=function(){$('#nextsection').on('click',function(){$target=$($(this).attr('href')).offset().top-30;$('body, html').animate({scrollTop:$target},750,'easeOutExpo');return!1})}
BRUSHED.goUp=function(){$('#goUp').on('click',function(){$target=$($(this).attr('href')).offset().top-30;$('body, html').animate({scrollTop:$target},750,'easeOutExpo');return!1})}
BRUSHED.scrollToTop=function(){var windowWidth=$(window).width(),didScroll=!1;var $arrow=$('#back-to-top');$arrow.click(function(e){$('body,html').animate({scrollTop:"0"},750,'easeOutExpo');e.preventDefault()})
$(window).scroll(function(){didScroll=!0});setInterval(function(){if(didScroll){didScroll=!1;if($(window).scrollTop()>1000){$arrow.css('display','block')}else{$arrow.css('display','none')}}},250)}
